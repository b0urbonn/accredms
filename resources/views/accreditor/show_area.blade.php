...
@section('content')
@php
    $canManageParameters = auth()->user()->isAdmin() || (auth()->user()->isFaculty() && auth()->user()->areas()
        ->where('areas.id', $area->id)
        ->wherePivotIn('assignment_role', ['handler', 'co-handler', 'member'])
        ->exists());
    $canReviewEvidence = auth()->user()->isAccreditor() && auth()->user()->areas()
        ->where('areas.id', $area->id)
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb breadcrumb-custom">
        ...
        <li class="breadcrumb-item"><a href="{{ route('accreditor.browse') }}">{{ auth()->user()->isFaculty() ? 'My Areas' : 'Browse Areas' }}</a></li>
...
<div class="area-detail-header mb-4">
    ...
    <div class="area-detail-actions">
        ...
        {{-- <a href="{{ route('accreditor.evaluation_report', $area) }}" class="btn btn-outline-primary" target="_blank">
            <i class="bi bi-clipboard2-check me-1"></i> Accreditor Evaluation
        </a> --}}
    ...
    @forelse($area->parameters as $index => $param)
        ...
        <section class="criterion-card-dark parameter-card mb-4" id="parameter-{{ $param->id }}">
            ...
            <div class="parameter-card-header d-flex flex-wrap align-items-center justify-content-between gap-3 p-3">
                ...
                <!-- Middle Column: UI/UX Professional Progress Widget (Hidden for Accreditors) -->
                @if(!auth()->user()->isAccreditor())
                ...
                <div class="parameter-actions">
                    @if($canManageParameters)
                        <button type="button" class="btn btn-xs btn-outline-dark"
                            onclick="openEditParameterModal({{ $param->id }}, '{{ addslashes($param->code) }}', '{{ addslashes($param->title) }}', '{{ addslashes($param->description ?? '') }}', {{ $param->sort_order ?? 0 }})">
                            <i class="bi bi-pencil"></i> Edit
                        </button>
                        @if($parameterHasData)
                            <button type="button" class="btn btn-xs btn-outline-danger" disabled
                                title="Delete is unavailable while this parameter has statements, sub-items, or uploaded documents.">
                                <i class="bi bi-trash"></i> Delete
                            </button>
                        @else
                            <form action="{{ route('parameters.destroy', $param) }}" method="POST" onsubmit="return confirm('Delete empty Parameter {{ addslashes($param->code) }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-xs btn-outline-danger" title="Delete parameter">
                                    <i class="bi bi-trash"></i> Delete
                                </button>
                            </form>
                        @endif
                    ...
                    <div class="category-header mb-2">
                        ...
                        @if(auth()->user()->isAdmin() || auth()->user()->isFaculty())
                            <div class="d-flex align-items-center gap-1">
                                <button type="button" class="btn btn-xs category-add-button" onclick="openAddSubfolderModal({{ $paramCat->id }}, '{{ addslashes($paramCat->category->name) }}')">
                        ...
                        <table class="table table-matrix-dark align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($paramCat->subfolders->sort(fn($a, $b) => strnatcasecmp($a->code ?? '', $b->code ?? '')) as $subfolder)
                                    @include('accreditor._subfolder_row', ['subfolder' => $subfolder, 'depth' => 0, 'paramCat' => $paramCat])
                                @empty
                                    <tr>
...
@if($canManageParameters)
    <div class="modal fade" id="createParameterModal" tabindex="-1" aria-labelledby="createParameterModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('parameters.store', $area) }}" class="modal-content">
                @csrf
                <div class="modal-header bg-apple-dark text-white">
                    <h5 class="modal-title fw-bold" id="createParameterModalLabel"><i class="bi bi-plus-circle me-1"></i> Add Parameter</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-semibold fs-7">Parameter Code</label>
                        <input type="text" name="code" class="form-control" maxlength="20" placeholder="e.g. 1.1" required>
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer bg-light py-2">
                    ...
                    <button type="submit" class="btn btn-sm btn-apple-green"><i class="bi bi-plus-lg me-1"></i> Create Parameter</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal fade" id="editParameterModal" tabindex="-1" aria-labelledby="editParameterModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form id="editParameterForm" method="POST" action="" class="modal-content">
                ...
                @method('PUT')
                <div class="modal-header bg-apple-dark text-white">
                    <h5 class="modal-title fw-bold" id="editParameterModalLabel"><i class="bi bi-pencil-square me-1"></i> Edit Parameter</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-semibold fs-7">Parameter Code</label>
                        <input type="text" id="editParameterCode" name="code" class="form-control" maxlength="20" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold fs-7">Parameter Title</label>
                        <input type="text" id="editParameterTitle" name="title" class="form-control" maxlength="255" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold fs-7">Description</label>
                        <textarea id="editParameterDescription" name="description" class="form-control" rows="3"></textarea>
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-semibold fs-7">Sort Order</label>
                        <input type="number" id="editParameterSortOrder" name="sort_order" class="form-control" min="0">
                    </div>
                    <input type="hidden" name="status" value="active">
                </div>
                <div class="modal-footer bg-light py-2">
                    <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-sm btn-apple-green"><i class="bi bi-check-lg me-1"></i> Save Changes</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- Modal: Evaluate Statement Sub-Item -->
<div class="modal fade" id="remarkModal" tabindex="-1">
    <div class="modal-dialog">
        <form id="remarkForm" method="POST" action="" class="modal-content">
            @csrf
            @method('PUT')
            <div class="modal-header bg-apple-dark text-white">
                <h5 class="modal-title fw-bold" id="remarkModalTitle"><i class="bi bi-clipboard2-check me-1"></i> Evaluate Statement Sub-Item</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p class="text-muted fs-7 mb-3">Statement sub-item: <strong id="remarkDocTitle"></strong></p>
                <div class="alert alert-light border fs-8 py-2 mb-3">
                    This assessment applies to the entire statement and appears in the Accreditor Evaluation Report. Use PDF findings for comments about an individual file.
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold fs-7">Statement Rating</label>
                    <select name="rating" id="remarkRating" class="form-select" required>
                        <option value="">Select survey rating</option>
                    ...
                    <small class="text-muted fs-8">One rating is saved per accreditor for this statement, regardless of how many PDF files it contains.</small>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold fs-7">Compliance Result</label>
                    <select name="compliance_result" id="remarkComplianceResult" class="form-select" required>
                ...
                <div class="mb-0">
                    <label class="form-label fw-semibold fs-7">Statement Assessment / Recommendation</label>
                    <textarea name="evaluation" id="remarkEvaluation" class="form-control" rows="3" required placeholder="Explain this statement rating and compliance result."></textarea>
                </div>
            </div>
            <div class="modal-footer bg-light py-2">
                <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="submit" class="btn btn-sm btn-apple-green" id="remarkSubmitButton">Save Evaluation</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal fade" id="additionalDocumentRequestModal" tabindex="-1">
    <div class="modal-dialog">
        <form id="additionalDocumentRequestForm" method="POST" action="" class="modal-content">
            @csrf
            <div class="modal-header bg-apple-dark text-white">
                <h5 class="modal-title fw-bold"><i class="bi bi-file-earmark-plus me-1"></i> Request Additional Documents</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p class="text-muted fs-7 mb-2">Statement: <strong id="additionalRequestItemTitle"></strong></p>
                <div class="alert alert-light border fs-8 py-2 mb-3">
                    <strong>Current checklist</strong>
                    <div id="additionalRequestChecklist" class="mt-1 text-secondary"></div>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold fs-7">Missing Documents / Details <span class="text-muted fw-normal">(optional)</span></label>
                    <textarea name="requested_documents" class="form-control" rows="3" placeholder="List specific missing documents or evidence details."></textarea>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold fs-7">Instructions / Remarks</label>
                    <textarea name="remarks" class="form-control" rows="3" required placeholder="State what Faculty needs to provide or revise."></textarea>
                </div>
                <div class="mb-0">
                    <label class="form-label fw-semibold fs-7">Due Date <span class="text-muted fw-normal">(optional)</span></label>
                    <input type="date" name="due_date" class="form-control">
                </div>
            </div>
            <div class="modal-footer bg-light py-2">
                <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="submit" class="btn btn-sm btn-apple-green"><i class="bi bi-send me-1"></i> Send Request</button>
            </div>
        </form>
    </div>
</div>
...
<style>
    ...
    .area-code { background: var(--accent-light); border: 1px solid #cfe1b9; border-radius: 4px; color: var(--accent-text); font-size: 0.72rem; font-weight: 700; padding: 0.25rem 0.45rem; }
    .area-detail-actions, .parameter-actions { align-items: center; display: flex; flex-wrap: wrap; gap: 0.5rem; }
    .area-filter-bar { background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 8px; box-shadow: var(--card-shadow); padding: 0.6rem; }
    .parameter-card { overflow: hidden; padding: 0; }
    .parameter-card.is-collapsed .category-block { display: none !important; }
    .parameter-card-header { align-items: flex-start; border-bottom: 1px solid var(--border-color); display: flex; gap: 1rem; justify-content: space-between; padding: 1.25rem 1.35rem; }
    .parameter-card-header h4, .parameter-card-header h5 { color: var(--text-primary); font-size: 1.2rem; font-weight: 800; margin-bottom: 0.25rem; }
    ...
    tr[id^="subfolder-"] { scroll-margin-top: 86px; }
...
<script>
    ...
    function scrollToStatementFromHash() {
        const statementId = window.location.hash.slice(1);
        const statement = statementId ? document.getElementById(statementId) : null;

        if (!statement) {
            return;
        }

        statement.scrollIntoView({ block: 'start' });
        window.requestAnimationFrame(() => {
            const headerHeight = document.querySelector('.top-navbar')?.offsetHeight ?? 0;
            window.scrollBy({ top: -(headerHeight + 16) });
        });
    }
    ...
    const requestedDocumentId = new URLSearchParams(window.location.search).get('request_id');
    if (requestedDocumentId) {
        const requestButton = document.querySelector('[data-open-request-id="' + requestedDocumentId + '"]');
        if (requestButton) {
            requestButton.click();
        }
    }

    window.addEventListener('load', scrollToStatementFromHash);
    ...
    function openEditParameterModal(id, code, title, description, sortOrder) {
        document.getElementById('editParameterForm').action = '/parameters/' + id;
        document.getElementById('editParameterCode').value = code;
        document.getElementById('editParameterTitle').value = title;
        document.getElementById('editParameterDescription').value = description;
        document.getElementById('editParameterSortOrder').value = sortOrder;
    ...
    function openItemEvaluationModal(button) {
        const isEditing = Boolean(button.dataset.rating);
        document.getElementById('remarkDocTitle').innerText = button.dataset.itemTitle;
        document.getElementById('remarkForm').action = button.dataset.action;
        document.getElementById('remarkModalTitle').innerHTML = '<i class="bi bi-clipboard2-check me-1"></i>' + (isEditing ? 'Edit Item Evaluation' : 'Evaluate Statement Sub-Item');
        document.getElementById('remarkRating').value = button.dataset.rating || '';
        document.getElementById('remarkComplianceResult').value = button.dataset.complianceResult || '';
        document.getElementById('remarkEvaluation').value = button.dataset.evaluation || '';
        document.getElementById('remarkSubmitButton').textContent = isEditing ? 'Update Evaluation' : 'Save Evaluation';
        new bootstrap.Modal(document.getElementById('remarkModal')).show();
    }

    function openAdditionalDocumentRequestModal(button) {
        document.getElementById('additionalDocumentRequestForm').action = button.dataset.action;
        document.getElementById('additionalRequestItemTitle').textContent = button.dataset.itemTitle;
        document.getElementById('additionalRequestChecklist').textContent = button.dataset.checklist || 'No checklist has been provided for this statement.';
        new bootstrap.Modal(document.getElementById('additionalDocumentRequestModal')).show();
    }

    @include('components.evidence-photo-capture-script')
